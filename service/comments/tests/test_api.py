...
class CommentListTests(TestCase):
    ...
    def test_requires_target_type_and_target_id(self) -> None:
        ...
        self.assertIn("target_id", resp.data)

    def test_rejects_invalid_target_type(self) -> None:
        self.client.force_authenticate(self.user)
        resp = self.client.get(self.url, {"target_type": "invalid", "target_id": self.event.id})
        self.assertEqual(resp.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertIn("target_type", resp.data)
...
class CommentCreateTests(TestCase):
    ...
    def test_create_comment_successfully(self) -> None:
        ...
        self.assertEqual(resp.data["body"], "Great event!")
        self.assertEqual(resp.data["author_id"], str(self.user.id))
    ...
    def test_create_with_parent(self) -> None:
        ...
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.assertEqual(resp.data["parent_id"], str(parent.id))
...
class CommentRetrieveTests(TestCase):
    ...
    def test_retrieve_comment(self) -> None:
        ...
        resp = self.client.get(self.url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertEqual(resp.data["id"], str(self.comment.id))
        self.assertEqual(resp.data["body"], "Read me")
        self.assertEqual(resp.data["author_id"], str(self.user.id))
        self.assertEqual(resp.data["target_type"], "events.event")
        self.assertEqual(resp.data["target_id"], str(self.event.id))
    ...
    def test_soft_deleted_returns_body_null_and_is_deleted_true(self) -> None:
        ...
        resp = self.client.get(self.url)
        self.assertEqual(resp.status_code, status.HTTP_200_OK)
        self.assertIsNone(resp.data["body"])
        self.assertTrue(resp.data["is_deleted"])
...
class CommentStaffModerationTests(TestCase):
    ...
    def test_event_creator_without_staff_cannot_moderate_others_comments(self) -> None:
        ...
        self.client.force_authenticate(event_creator)
