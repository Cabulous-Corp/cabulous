...
def test_send_simple_email_task(mock_svc) -> None:
...
def test_send_discord_channel_message_task(mock_svc) -> None:
    send_discord_channel_message_task(
        webhook_url="https://hooks.test", content="hi",
    )
...
@patch("communication.tasks.DiscordService")
def test_send_discord_channel_message_by_purpose_task(mock_svc) -> None:
    send_discord_channel_message_by_purpose_task(
        purpose="alerts", content="hello",
    )
...
@patch("communication.tasks.DiscordService")
def test_send_discord_channel_message_from_template_task(mock_svc) -> None:
    send_discord_channel_message_from_template_task(
...
@patch("communication.tasks.DiscordService")
def test_send_discord_channel_message_from_template_by_purpose_task(mock_svc) -> None:
    send_discord_channel_message_from_template_by_purpose_task(
        purpose="alerts",
        template_path="discord/alert.html",
        context={"env": "prod"},
...
@patch("communication.tasks.DiscordService")
def test_send_discord_channel_embed_task(mock_svc) -> None:
    send_discord_channel_embed_task(
        webhook_url="https://hooks.test",
        content="hi",
        embeds=[{"title": "Test"}],
...
@patch("communication.tasks.DiscordService")
def test_send_discord_channel_embed_by_purpose_task(mock_svc) -> None:
    send_discord_channel_embed_by_purpose_task(
        purpose="alerts",
        content="hi",
        embeds=None,
    ...
    mock_svc.send_channel_embed_by_purpose.assert_called_once_with(
        purpose="alerts", content="hi", embeds=None,
    )
